...
class SPSParser {
    ...
    static parseHEVCSPS(uint8array) {
        ...
        let max_sub_layers = gb.readBits(3) + 1;
        ...
        let bit_depth_luma = gb.readUEG() + 8;
        let bit_depth_chroma = gb.readUEG() + 8;
        let log2_max_pic_order_cnt_lsb = gb.readUEG() + 4;
        let sub_layer_ordering_info_present_flag = gb.readBool();
        for (let i = sub_layer_ordering_info_present_flag ? 0 : max_sub_layers - 1; i <= max_sub_layers - 1; i++) {
            gb.readUEG();
            gb.readUEG();
            gb.readUEG();
        }
        let log2_min_luma_coding_block_size = gb.readUEG() + 3;
        let log2_diff_max_min_luma_coding_block_size = gb.readUEG();
        let log2_min_transform_block_size = gb.readUEG() + 2;
        ...
        if (long_term_ref_pics_present_flag) {
            ...
            for (let i = 0; i < num_long_term_ref_pics_sps; i++) {
                for (let j = 0; j < 2 ** log2_max_pic_order_cnt_lsb; j++) { gb.readBits(); }
                gb.readBits();
            }
        }
        ...
        let vui_parameters_present_flag = gb.readBool();
        let min_spatial_segmentation_idc = null
        let parallelismType = null
        let chromaFormat = null
        ...
        if (vui_parameters_present_flag) {
            ...
            if (aspect_ratio_info_present_flag) {
                ...
                if (aspect_ratio_idc === 255) {
                    let sar_width = gb.readBits(16);
                    let sar_height = gb.readBits(16);
                }
            }
            ...
            let default_display_window_flag = gb.readBool();
            ...
            if (vui_timing_info_present_flag) {
                ...
                if (vui_poc_proportional_to_timing_flag) {
                    gb.readUEG();
                    vui_hrd_parameters_present_flag = gb.readBool();
            ...
            if (bitstream_restriction_flag) {
                let tiles_fixed_structure_flag = fb.readBool()
        ...
        let sps_extension_flag = gb.readBool();
        while (sps_extension_flag) {
            // ignore
        }
        */

        let codec_mimetype = 'hvc1';
