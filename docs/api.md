...
| `liveSyncTargetLatency?`         | `number`  | `0.8`                        | Target latency in HTMLMediaElement to chase when latency exceeds liveSyncMaxLatency, in seconds. Effective only if `isLive: true` and `liveSync: true` |
